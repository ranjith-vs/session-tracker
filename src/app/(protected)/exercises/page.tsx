...
export default function ExercisesPage() {
  ...
  const [modalExercise, setModalExercise] = useState<Exercise | null | undefined>(undefined) // undefined = closed
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{exercises.length} exercise{exercises.length !== 1 ? 's' : ''} in your library</p>
        </div>
        ...
        <div className="card p-8 text-center">
          {search ? (
            ...
            <>
              ...
              <button onClick={() => setModalExercise(null)} className="btn-primary mt-4">
            ...
            <div key={exercise.id} className="card p-4 flex items-center justify-between">
              <div className="min-w-0 flex-1">
                ...
                {exercise.description && (
                  <p className="text-sm text-gray-400 mt-0.5 truncate">{exercise.description}</p>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-sm p-6">
            ...
            <p className="text-gray-400 text-sm mb-6">
              This will permanently delete the exercise. Associated workout logs will remain but the exercise name may be unavailable.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)} className="btn-secondary flex-1">Cancel</button>
              <button onClick={() => handleDelete(deleteId)} className="flex-1 bg-red-600 hover:bg-red-500 text-white font-medium px-4 py-2 rounded-lg transition-colors">
                Delete
              </button>
            </div>
          </div>
